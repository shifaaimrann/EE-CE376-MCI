...
float derivative_filtered = 0.0f;  // global
...
#define PWM_MAX             60.0f
#define PWM_MIN_START       15.0f
...
void imu_process(void)
{
 ...
 gyro_read_raw();
 gyro_convert(); gyro_rate_filtered = 0.7f * gyro_rate_filtered + 0.3f * gyro.y_dps;
 gyro_rate = gyro_rate_filtered;
...
void pid_compute_and_drive(void)
{
 ...
 integral += error * DT;
 integral = clamp_float(integral, -10.0f, 10.0f);

 // derivative = (error - prev_error) / DT;
 float raw_deriv = (error - prev_error) / DT;
 derivative_filtered = 0.85f * derivative_filtered + 0.15f * raw_deriv;
 derivative = derivative_filtered;
...
/* USER CODE BEGIN 2 */
 ...
 pid_output = 0.0f;
  ...
  // HAL_Delay(2000);        /* 2 second wait so you can get multimeter ready */
  // Motors_Backward(60);    /* force backward at speed 60 */
  // HAL_Delay(5000);        /* hold backward for 5 seconds */
  // Motors_Stop();
