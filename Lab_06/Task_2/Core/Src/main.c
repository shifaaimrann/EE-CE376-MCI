...
/* USER CODE BEGIN PV */
// 48 MHz timer clock, PSC = 47  => 1 us per tick
...
// --------- TIM2 CH1 Input Capture callback (PD3) ----------
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
  if (htim->Instance == TIM2 && htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1)
  {
    ...
    else
    {
      ...
      else           period_us = (ARR_MAX - ic1) + ic2 + 1;  // overflow
...
int main(void)
{
  ...
  MX_USART2_UART_Init();
  MX_USB_PCD_Init();
  myprintf("Task2: TIM2_CH1 Input Capture on PD3\r\n");
  myprintf("Clock=48MHz, PSC=47 => 1us tick\r\n");
...
// Start input capture interrupt on TIM2 CH1
    ...
    if (new_data)
  {
    ...
    if ((HAL_GetTick() - last_print) >= 100 && period_us > 0)
    {
      last_print = HAL_GetTick();

      // f = 1,000,000 / period_us   (integer Hz)
      uint32_t f_hz = TIMER_HZ / period_us;
